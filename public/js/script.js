...
function handleCardClick(event) {
  ...
  if (clickCount < 2 && preClass != event.target) {
    ...
    if (!preClass) {
      ...
      preClass.className.split(' ')[0] == event.target.className.split(' ')[0]
    ) {
      event.target.style.backgroundImage = `url("${
        event.target.className.split(' ')[0]
      }")`;
      ...
      if (
        (colorArray.length == 3 && checkLevel == '1') ||
        (colorArray.length == 4 && checkLevel == '2') ||
        (colorArray.length == 8 && checkLevel == '3')
      ) {
      ...
      setTimeout(() => {
        ...
        event.target.style.backgroundImage =
          'linear-gradient(to right bottom, violet, blue, white)';

        preClass.style.backgroundImage = 'none';
        preClass.style.backgroundImage =
          'linear-gradient(to right bottom, violet, blue, white)';
...
function resetButtonClick(event) {
  ...
  for (let index = 0; index < game.length; index++) {
    ...
    game[index].style.backgroundImage =
      'linear-gradient(to right bottom, violet, blue, white)';
  }
}
